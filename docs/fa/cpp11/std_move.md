...
- **`lvalue`:** عبارتی که به یک شیء با هویت مشخص در حافظه اشاره دارد و معمولاً می‌توان بعداً نیز به آن دسترسی داشت.
- **`rvalue`:** مقدار یا شیء موقتی که معمولاً فقط در همان عبارت وجود دارد و عمر کوتاهی دارد.
...
چون `x` یک شیء با مکان مشخص در حافظه است.
...
در این حالت، اگر `std::string` move constructor داشته باشد، ممکن است عملیات move انجام شود.
...
- `v2` داده‌های قبلی `v1` را دارد.
- `v1` در وضعیت معتبر ولی نامشخص قرار دارد.
...
| مرجع به rvalue | `void foo(std::string&& s);` | rvalueها را می‌پذیرد و پایه‌ای برای Move Semantics است. |
...
`std::move` به‌تنهایی هیچ داده‌ای را جابه‌جا نمی‌کند. این دستور فقط عبارت را به شکل rvalue درمی‌آورد.
...
### 2. فکر کنیم شیء moveشده حتماً خالی یا `nullptr` است
...
| مفهوم | توضیح |
|---|---|
| `lvalue` | عبارتی با هویت مشخص که معمولاً می‌توان به آن ارجاع داد و آدرس آن را گرفت. |
| `rvalue` | مقدار یا شیء موقتی که معمولاً عمر کوتاهی دارد و می‌توان از آن move کرد. |
| `prvalue` | مقدار موقتی بدون هویت، مثل `5` یا نتیجه تابعی که by value برمی‌گردد. |
| `xvalue` | مقدار در حال انقضا، مثل نتیجه `std::move(x)`. |
| `&&` | rvalue reference؛ پایه‌ای برای Move Semantics. |
| `std::move` | cast برای تبدیل عبارت به rvalue reference؛ به‌تنهایی چیزی را move نمی‌کند. |
| Move Constructor | ساخت شیء جدید با انتقال منابع از یک شیء موجود. |
| Move Assignment | جایگزینی منابع شیء مقصد با منابع شیء منبع بدون copy پرهزینه. |
| Copy Constructor | ساخت شیء جدید با کپی از شیء موجود. |
| Copy Assignment | جایگزینی محتوای شیء مقصد با کپی از شیء منبع. |
| Moved-from object | شیء moveشده معمولاً معتبر ولی نامشخص است. |
...
- این مقاله با استفاده از توضیحات و بازنویسی انجام‌شده توسط **ChatGPT (OpenAI)** و **Qwen (Alibaba)** تهیه و ویرایش شده است.
