...
بله. این یکی از مهم‌ترین مفاهیم C++ مدرن (از C++11 به بعد) است. اگر **lvalue** و **rvalue** را خوب درک کنی، فهمیدن **move semantics** و **rvalue references** خیلی راحت‌تر می‌شود.

---

# اول: lvalue و rvalue یعنی چه؟

به زبان ساده:

* **lvalue**: چیزی که **مکان مشخصی در حافظه دارد** و می‌توان بعداً دوباره به آن مراجعه کرد.
* **rvalue**: یک **مقدار موقتی (temporary)** که معمولاً فقط در همان عبارت (expression) وجود دارد.
...
امکان‌پذیر است؛ یعنی `x` یک محل در حافظه دارد.
...
یعنی حتی شیء موقتی هم کپی می‌شود.

در حالی که آن شیء دیگر قرار نیست استفاده شود.

این اتلاف حافظه و زمان است.

---

# Move Semantics چه می‌گوید؟

ایده بسیار ساده است:

اگر شیء موقتی است،

دیگر لازم نیست از آن **کپی** بگیریم.

می‌توانیم منابعش را **بدزدیم (steal)**.
...
فقط اشاره‌گر منتقل می‌شود.

هیچ داده‌ای کپی نمی‌شود.

---

# این کار چگونه ممکن شد؟

C++11 چیزی به نام **rvalue reference** معرفی کرد.
...
این تابع فقط rvalue قبول می‌کند.
...
در واقع نمی‌گوید چیزی را جابه‌جا کن.

بلکه فقط به کامپایلر می‌گوید:
...
پیچیدگی زمانی تقریباً از **O(n)** به **O(1)** کاهش پیدا می‌کند.
...
| `std::move`      | شیء را جابه‌جا نمی‌کند؛ فقط آن را به صورت یک rvalue معرفی می‌کند تا در صورت امکان، عملیات move انجام شود. |
| Move Constructor | منابع شیء مبدأ را به شیء جدید منتقل می‌کند و از کپی پرهزینه جلوگیری می‌کند.                               |
| Move Assignment  | منابع را در انتساب منتقل می‌کند و معمولاً سریع‌تر از کپی است.                                             |
